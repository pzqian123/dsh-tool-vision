...
/**
 * Report whether the CALLING agent's current route can already receive images.
 * When it can, forwarding the image to the configured vision model would be
 * redundant (and would ship bytes to a provider the main model could have read
 * directly), so the tool refuses with guidance to use the built-in `read_image`
 * instead — mirroring how `read_image` refuses on text-only routes.
 * Unknown/unresolvable routes never gate (returns false).
 */
async function callingRouteAcceptsImages(llm, exec) {
	const config = exec.agent?.session?.header?.config;
	if (config === void 0) return false;
	const { provider, model } = config;
	if (typeof provider !== "string" || provider.length === 0 || typeof model !== "string" || model.length === 0) return false;
	try {
...
${value.description}${value.truncated ? "\n\n(Description truncated at the vision model's max output tokens; call again with a narrower prompt if you need the rest.)" : ""}
		...
		async execute(args, exec) {
			...
			if (await callingRouteAcceptsImages(llm, exec)) {
				const header = exec.agent?.session?.header?.config;
				throw new Error(`your current model route (${header.provider}/${header.model}) already accepts image input; use the built-in read_image tool instead of forwarding the image to the configured vision model (${route.provider}/${route.model})`);
			}
...
function apply(ctx, config) {
	...
	ctx.systemPrompt.section({
		name: "tool:vision",
		order: 100,
		text: "If your current model route cannot receive images (e.g. a text-only model), read an image by calling the read_image_vision tool: it sends the image to the configured vision-capable model together with the task-specific `prompt` you pass. The vision model sees ONLY the image and that prompt — it has no access to this conversation — so write the prompt with every detail the answer depends on (e.g. 'Transcribe all visible text verbatim', 'Describe the layout and colors', 'Read the numbers in the table'). Use the returned description to continue the task. If your model route can receive images directly, prefer the built-in read_image tool instead."
	});
